...
All EasyResearch code lives under `src/easyresearch/`:

- **components/** — All UI components. The most important ones:
  - `SurveyBuilder.tsx` — The main editor with 6 tabs (Questionnaires, Logic, Layout, Settings, Preview, Responses)
  - `QuestionnaireList.tsx` — CRUD for questionnaire records plus inline question editing
  - `ParticipantTypeManager.tsx` — Manages participant types with consent and screening
  - `LayoutBuilder.tsx` — Drag-and-drop mobile app layout builder
  - `SurveySettings.tsx` — Project settings (basic info, features, ecogram, schedule, capacity, compensation)
  - `ParticipantOnboarding.tsx` — Multi-step participant enrollment flow
  - `ESMParticipantDashboard.tsx` — Longitudinal study participant dashboard
  - `EcogramBuilder.tsx` — Interactive care network diagram
  - `EasierResearchLayout.tsx` — Researcher sidebar layout
  - `layouts.ts` — PublicLayout, ResearcherLayout, ParticipantLayout wrappers
- **constants/questionTypes.ts** — Single source of truth for all 21 supported question types
- **services/** — Backend service modules:
  - `aiService.ts` — AI entitlements, credit system, voice processing
  - `notificationService.ts` — Capacitor local notification scheduling
  - `templateService.ts` — 15 pre-built templates and project creation from template
  - `validationService.ts` — Response validation engine and React hook

---

## 3. ROUTE MAP

### 3.1 Public Routes (no auth, header only, no sidebar)

- `/easyresearch` — Landing page
- `/easyresearch/auth` — Login/signup (accepts redirect params)
- `/easyresearch/templates` — Template library
- `/easyresearch/pricing` — Pricing tiers

### 3.2 Researcher Routes (auth-protected, header plus sidebar)

- `/easyresearch/dashboard` — Project list and stats (pass `?create=true` to auto-open create dialog)
- `/easyresearch/create-survey` — New project builder
- `/easyresearch/project/:projectId` — Edit existing project
- `/easyresearch/project/:projectId/responses` — Per-project responses
- `/easyresearch/responses` — Cross-project responses
- `/easyresearch/participants` — Cross-project participant management
- `/easyresearch/settings` — Researcher account settings

### 3.3 Participant Routes (header plus bottom nav)

- `/easyresearch/home` — Enrolled studies list
- `/easyresearch/participant/join` — Join via survey code or invite token
- `/easyresearch/participant/:projectId` — Survey view (routed by project type)
- `/easyresearch/participant/:projectId/dashboard` — ESM timeline and compliance
- `/easyresearch/participant/:projectId/settings` — Participant settings
- `/easyresearch/survey/:projectId/complete` — Thank-you page

### 3.4 Catch-all

- `/easyresearch/*` — Redirects to `/easyresearch`

---

## 4. LAYOUT WRAPPERS

Three layout wrappers defined in `layouts.ts`, used in `App.tsx`:

- **PublicLayout** — Header only, no sidebar. For landing, auth, templates, pricing.
- **ResearcherLayout** — Header plus left sidebar with nav items (Dashboard, Create Survey, Participants, Responses, Analytics, Settings). Shows user email and "Researcher Account" label. Protected by the RequireResearcher guard.
- **ParticipantLayout** — Header plus bottom navigation bar. For all participant-facing views.

---

## 5. AUTH AND ROLE SYSTEM

### 5.1 RequireResearcher Guard

Defined in `App.tsx`. Checks the `researcher` table for a row matching the current auth user. If no user is logged in, redirects to auth. If the user exists but has no researcher record, redirects to auth with a "become researcher" flag.

### 5.2 Organization and Researcher Hierarchy

Each organization has a name, slug, plan tier (free, professional, team, enterprise), and an AI credit balance. Each researcher belongs to one organization and has a role. One org can have multiple researchers. If a user hits researcher routes without a record, the system auto-creates an org and researcher on demand.

### 5.3 Participant Join Flow

Two ways to join a study:
- **Survey code** — Enter the 6-character code. Looks up the project, creates an enrollment.
- **Invite token** — URL with token and project ID. Validates the token, activates the enrollment.

If not logged in, the pending code is saved to session storage and the user is redirected to auth. After login, the join auto-completes.

---

## 6. RESEARCH PROJECT — THE CORE RECORD

Every study lives in the `research_project` table. A project holds the title, description, type, status, and all global settings. It belongs to one organization and one researcher.

- **Project types:** one-time survey, longitudinal, ESM, clinical trial, diary study
- **Statuses:** draft, published, active. Publishing generates a unique 6-character survey code.
- **Feature flags:** AI assist, voice input, notifications, onboarding required
- **Schedule:** start date, end date, study duration in days, survey frequency, allow-participant-DND toggle
- **Compensation:** type (none, monetary, gift card, raffle) and amount
- **Sampling:** proper columns — sampling type, prompts per day, start/end hour, allow late flag, late window minutes
- **Notification:** proper columns — frequency, times per day, specific notification times array, send reminders flag, timezone

### 6.1 No More JSONB Blobs

The old `setting` JSONB column has been **completely removed**. All fields that used to live in JSONB are now proper typed columns on the `research_project` table: `show_progress_bar`, `disable_backtracking`, `randomize_questions`, `auto_advance`, `consent_required`, `consent_form_title`, `consent_form_text`, `consent_form_url`, `screening_enabled`, `participant_number_prefix`, `participant_relation_enabled`, `participant_relation_options`, `ecogram_enabled`, `ecogram_center_label`, `ecogram_relationship_options`, `ecogram_support_categories`, `notification_frequency`, `notification_times_per_day`, `notification_times`, `notification_send_reminders`, `notification_timezone`, `sampling_type`, `sampling_prompts_per_day`, `sampling_start_hour`, `sampling_end_hour`, `sampling_allow_late`, `sampling_late_window_minutes`, `recruitment_criteria_text`.

The following JSONB columns were also dropped: `consent_form`, `notification_setting`, `sampling_strategy`, `recruitment_criteria`, `timeline_config`, `profile_question`. Their data is either flattened into proper columns above or moved to separate tables (`profile_question` table, `logic_rule` table).

**The only remaining JSONB column is `app_layout`** — the mobile app layout config. This is a deeply nested UI structure (tabs → elements → configs) that would require 3+ extra tables to make fully relational, with no benefit since it's only read and written as a whole blob. Everything else is a proper typed column or a proper table.

---

## 7. SURVEYBUILDER — THE MAIN EDITOR

`SurveyBuilder.tsx` is the central editor component with 6 tabs:

1. **Questionnaires** — Create, edit, reorder, and duplicate questionnaires. Each has its own questions, schedule, and notification settings.
2. **Logic** — Define conditional skip and branching rules across all questions.
3. **Layout** — Drag-and-drop builder for the participant-facing mobile app layout.
4. **Settings** — Project title, description, type, participant types, consent/screening, ecogram, schedule, capacity, compensation.
5. **Preview** — Desktop and mobile preview with a phone mockup.
6. **Responses** — View collected responses (only visible for saved projects).

### 7.1 How Data Loads

When opening an existing project, the builder fetches the project row, then separately fetches all questionnaire rows, participant type rows, and junction table assignments from their own tables. It also fetches all question rows and distributes them into their parent questionnaires using the `questionnaire_id` foreign key on each question.

### 7.2 How Data Saves

Saving runs three sync operations:
1. **Participant types** are upserted into the `participant_type` table. Removed types are deleted.
2. **Questionnaires** are upserted into the `questionnaire` table. The junction table is rebuilt for each questionnaire's assigned participant types.
3. **Questions** are upserted into `survey_question` with the proper `questionnaire_id` FK set. Options go into `question_option`.

The project row itself is updated with display settings and the `app_layout` JSONB column.

### 7.3 Publish Flow

Publishing validates that at least one question exists and the project has a title. It generates a survey code if missing, sets status to published, and records the timestamp. Unpublishing reverts to draft.
