...
### Critical Requirement % Clarification Starts (CRC Starts)  ###

1. 特殊组件timeline（时间线组件)，时间线组件的目的是把不同的问卷按照设定的问卷回答时间显示在时间线上，时间线上显示待回答问题，是否已回答，时间线组件的设置允许研究者选择将哪些问题显示在时间线上

### Critical Requirement % Clarification Ends (CRC Ends)  ###
