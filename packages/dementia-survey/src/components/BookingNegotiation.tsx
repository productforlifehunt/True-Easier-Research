...
const BookingNegotiation: React.FC<BookingNegotiationProps> = ({ helper, onClose, onConfirm }) => {
  ...
  const sendProposal = async () => {
    try {
      ...
      await supabase.from('hfh_negotiations').insert(proposal);
      setNegotiationStatus('negotiating');
      toast.success('Proposal sent to helper!');
      
      // Simulate helper response after 2 seconds
      setTimeout(() => {
        if (proposedRate >= helper.hourly_rate * 0.9) {
          setNegotiationStatus('accepted');
          toast.success('Helper accepted your proposal!');
        } else {
          setCounterOffer({
            rate: helper.hourly_rate * 0.95,
            message: `I appreciate your interest! How about $${(helper.hourly_rate * 0.95).toFixed(0)}/hour?`
          });
        }
      }, 2000);
    } catch (error) {
