...
const CompletedSurveyView: React.FC<CompletedSurveyViewProps> = ({ instanceId }) => {
  ...
  const loadResponses = async () => {
    ...
    try {
      ...
      const instanceResult = await supabase
        ...
        .select('enrollment_id, project_id')
        .eq('id', instanceId)
        .single();

      if (instanceResult.error || !instanceResult.data) {
        console.error('Failed to load instance:', instanceResult.error);
        setLoading(false);
        return;
      }
      ...
      if (projectQuestions.length > 0) {
        ...
        // Build responses array with all questions
        ...
        responsesArray.forEach((r: SurveyResponse) => {
          const question = r.survey_questions;
          if (question && question.options && r.response_value) {
            try {
              // Parse response_value if it's JSON
              let values = r.response_value;
              ...
              // Update response_text with readable labels
              if (labels.length > 0) {
        ...
        responsesArray.forEach((r: SurveyResponse) => {
          ...
          initialValues[r.question_id] = r.response_value;
        });
  ...
  const handleSave = async () => {
    try {
      ...
      for (const response of responses) {
        ...
        if (newText !== response.response_text || JSON.stringify(newValue) !== JSON.stringify(response.response_value)) {
          await supabase
            .from('survey_responses')
            .update({ 
  ...
  const sortedQuestions = Object.entries(groupedResponses).sort((a, b) => {
    const orderA = a[1][0]?.survey_questions?.order_index || 0;
    const orderB = b[1][0]?.survey_questions?.order_index || 0;
            ...
            <button
              onClick={() => {
                ...
                responses.forEach((r: SurveyResponse) => {
                  initialEdits[r.question_id] = r.response_text;
                  initialVals[r.question_id] = r.response_value;
                });
                setEditedResponses(initialEdits);
              ...
              {questionType === 'text' ? (
                ...
                <div className="space-y-3">
                  {options.map((option: any) => {
                    ...
                    const currentValues = editMode ? (editedValues[response.question_id] || []) : (response.response_value || []);
                      ...
                      >
                        <div className="flex-shrink-0">
                          <input
                            ...
                            onChange={(e) => {
                              if (!editMode) return;
                              const currentVals = editedValues[response.question_id] || response.response_value || [];
                ...
                <div className="space-y-3">
                  {options.map((option: any) => {
                    const optionValue = option.value || option.id || option.text;
                    const isSelected = response.response_value === optionValue || response.response_text === option.text;
