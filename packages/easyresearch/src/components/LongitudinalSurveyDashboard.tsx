...
const LongitudinalSurveyDashboard: React.FC = () => {
  ...
  const calculateProgress = (
    ...
    if (sessions.length > 0) {
      ...
      if (canTakeSurvey && enrollment.dnd_settings) {
        const currentHour = now.getHours();
        const currentDay = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'][now.getDay()];
        
        for (const period of enrollment.dnd_settings) {
  ...
  return (
    ...
    <div className="min-h-screen py-8" style={{ backgroundColor: 'var(--bg-primary)', paddingBottom: '100px' }}>
      <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                Participant ID: <strong>{enrollment.participant_number}</strong>
              </p>
            </div>
        ...
        <div className={`rounded-2xl shadow-sm p-6 mb-6 ${progress.canTakeSurvey ? 'bg-green-50 border-2 border-green-500' : 'bg-white'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center ${progress.canTakeSurvey ? 'bg-green-500 text-white' : 'bg-gray-200'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl p-4">
            ...
            <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>
              {Math.round((progress.completedSurveys / progress.totalSurveys) * 100)}%
            </p>
        ...
        {enrollment.dnd_settings && enrollment.dnd_settings.length > 0 && (
          <div className="mt-6 bg-blue-50 rounded-xl p-4">
            <p className="text-sm font-medium mb-2" style={{ color: 'var(--text-primary)' }}>
              Your Quiet Hours:
            </p>
            <div className="space-y-1">
              {enrollment.dnd_settings.map((period: any, index: number) => (
