...
import { ArrowLeft, Plus, Save, Trash2, List } from 'lucide-react';
...
const MobileSurveyEditor: React.FC = () => {
  ...
  const loadProject = async () => {
    console.log('=== MOBILE SURVEY EDITOR LOAD PROJECT ===');
  ...
  const addQuestion = async (type: string) => {
    const newQuestion: Question = {
      ...
      required: false,
      options: ['single_choice', 'multiple_choice', 'likert', 'dropdown'].includes(type) ? [
        { id: crypto.randomUUID(), option_text: 'Option 1', order_index: 0 },
        { id: crypto.randomUUID(), option_text: 'Option 2', order_index: 1 }
      ] : undefined
    };
      ...
      for (const question of questions) {
        const { data: insertedQuestion } = await supabase
          ...
          .insert({
            ...
            response_required: question.response_required || 'optional',
            allow_other: question.allow_other || false,
            allow_none: question.allow_none || false,
            allow_voice: false,
              ...
              <div className="space-y-3 p-3 rounded-lg" style={{ backgroundColor: '#f0fdf4', border: '1px solid var(--border-light)' }}>
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={project.ai_enabled}
                      onChange={(e) => setProject({ ...project, ai_enabled: e.target.checked })}
                      className="rounded"
                    />
                    <span className="text-sm" style={{ color: 'var(--text-primary)' }}>Enable AI Assistance</span>
                  </label>
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
      ...
      <div className="px-4 py-6 space-y-4">
        {questions.map((question, index) => (
          ...
          >
            ...
            <div className="mb-3">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const needsOptions = ['single_choice', 'multiple_choice', 'likert', 'dropdown'];
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateQuestion(question.id, {
                        validation_rules: { ...question.validation_rules, min_length: e.target.value }
                      })}
                      placeholder="Optional"
                      className="w-full px-2 py-1 rounded border text-sm"
                      style={{ borderColor: 'var(--border-light)' }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateQuestion(question.id, {
                        validation_rules: { ...question.validation_rules, max_length: e.target.value }
                      })}
                      placeholder="Optional"
                      className="w-full px-2 py-1 rounded border text-sm"
                      style={{ borderColor: 'var(--border-light)' }}
            ...
            {(question.question_type === 'single_choice' || question.question_type === 'multiple_choice' || question.question_type === 'likert' || question.question_type === 'dropdown' || question.question_type === 'scale' || question.question_type === 'ranking') && (
            ...
            <div className="space-y-2 pt-3 border-t" style={{ borderColor: 'var(--border-light)' }}>
              ...
              {project.ai_enabled && (
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={question.allow_ai_assist !== false}
                    onChange={(e) => updateQuestion(question.id, { allow_ai_assist: e.target.checked })}
                    className="rounded"
                  />
                  <span className="text-xs" style={{ color: 'var(--text-primary)' }}>✨ Enable AI Enhancement</span>
                </label>
              )}
            </div>
            ...
            <div className="mt-3 p-3 rounded-lg bg-gray-50">
              ...
              <div className="mt-2">
                ...
                {['text_short', 'text_long', 'number', 'date', 'time', 'email', 'phone'].includes(question.question_type) && (
                  <input
