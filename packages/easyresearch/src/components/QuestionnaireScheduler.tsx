...
const QuestionnaireScheduler: React.FC<QuestionnaireSchedulerProps> = ({ 
  ...
  const loadData = async () => {
    try {
      console.log('=== QUESTIONNAIRE SCHEDULER LOAD DATA ===');
      ...
      console.log('Questionnaire error:', qError);
      console.log('Questionnaire count:', questionnaireData?.length || 0);

      if (qError) {
        console.error('ERROR LOADING QUESTIONNAIRES:', qError);
        console.error('Error details:', JSON.stringify(qError));
      }

      if (questionnaireData) {
        console.log('Setting questionnaires state with', questionnaireData.length, 'items');
        console.log('Questionnaire details:', JSON.stringify(questionnaireData));
        setQuestionnaires(questionnaireData);
      } else {
        console.warn('No questionnaire data returned, setting empty array');
        setQuestionnaires([]);
      }
      ...
      if (scheduleData) {
        console.log('Setting schedule state with', scheduleData.length, 'items');
        console.log('Schedule details:', JSON.stringify(scheduleData));
  ...
  const handleTapSchedule = async (e: React.MouseEvent, dayNumber: number, timeSlot: string) => {
    e.stopPropagation();
    console.log('=== TAP SCHEDULE ===');
    console.log('Selected questionnaire:', selectedQuestionnaire?.title);
    console.log('Day:', dayNumber, 'Time:', timeSlot);
    ...
    try {
      console.log('Inserting schedule item...');
      ...
      if (error) {
        ...
        console.log('Schedule item inserted successfully');
        await loadData();
  ...
  const pasteDaySchedule = async (targetDay: number) => {
    ...
    try {
      ...
      const newItems = itemsToCopy.map(item => ({
        ...
        notification_minutes_before: item.notification_minutes_before
      }));
  ...
  console.log('=== QUESTIONNAIRE SCHEDULER RENDER ===');
  console.log('Questionnaires in state:', questionnaires.length);
  console.log('Show library:', showLibrary);
  console.log('Show schedule:', showSchedule);
              ...
              <button
                onClick={async () => {
                  if (confirm('Delete this questionnaire? This will also remove all scheduled instances.')) {
                    ...
                    await supabase.from('questionnaires').delete().eq('id', editingQuestionnaire.id);
                    await loadData();
