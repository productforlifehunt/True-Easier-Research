...
const AvailabilityCalendar: React.FC<AvailabilityCalendarProps> = ({
  ...
  // Generate default time slots for each day (8 AM to 8 PM)
  const generateDefaultTimeSlots = (availableSlots: string[] = [], bookedSlots: string[] = []): TimeSlot[] => {
    ...
    for (let hour = 8; hour <= 20; hour++) {
      ...
      // Default to available if no availability data, otherwise check against available slots
      const isAvailable = availableSlots.length === 0 || availableSlots.includes(time);
      const isBooked = bookedSlots.includes(time);
      
      slots.push({ time, available: isAvailable && !isBooked, booked: isBooked });
      if (hour < 20) {
        const halfAvailable = availableSlots.length === 0 || availableSlots.includes(halfTime);
        const halfBooked = bookedSlots.includes(halfTime);
        slots.push({ time: halfTime, available: halfAvailable && !halfBooked, booked: halfBooked });
      }
    }
    return slots;
  };
  ...
  const loadAvailability = async () => {
    ...
    try {
      ...
      // Fetch helper's availability settings from database
      const { data: availabilityData } = await supabase
        .from('hfh_helper_availability')
        .select('date, time_slots')
        .eq('helper_id', helperId)
        .gte('date', daysInMonth[0]?.toISOString().split('T')[0])
        .lte('date', daysInMonth[daysInMonth.length - 1]?.toISOString().split('T')[0]);
      ...
      const { data: bookingsData } = await supabase
        ...
        .gte('booking_date', daysInMonth[0]?.toISOString().split('T')[0])
        .lte('booking_date', daysInMonth[daysInMonth.length - 1]?.toISOString().split('T')[0]);
      
      // Create lookup maps
      const availabilityMap = new Map<string, string[]>();
      availabilityData?.forEach(a => {
        availabilityMap.set(a.date, a.time_slots || []);
      });
      
      const bookingsMap = new Map<string, string[]>();
      bookingsData?.forEach(b => {
        const existing = bookingsMap.get(b.booking_date) || [];
        existing.push(b.booking_time);
        bookingsMap.set(b.booking_date, existing);
      });
      
      // Generate slots for each day
      daysInMonth.forEach(date => {
        const dateKey = date.toISOString().split('T')[0];
        const availableSlots = availabilityMap.get(dateKey) || [];
        const bookedSlots = bookingsMap.get(dateKey) || [];
        newAvailability[dateKey] = generateDefaultTimeSlots(availableSlots, bookedSlots);
      });
