...
const AnalyticsPage: React.FC = () => {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      const { data: researcher } = await supabase
        .from('researcher').select('id').eq('user_id', user.id).maybeSingle();
      const { data: projects } = await supabase
        .from('research_project').select('id').eq('researcher_id', researcher?.id);
      ...
      const { data: enrollments } = await supabase
        .from('enrollment').select('id, participant_id, participant_email, status, project_id').in('project_id', projectIds);
      ...
      const { data: responses } = await supabase
        .from('survey_respons').select('id, created_at, question_id, enrollment_id, response_text, response_value')
      ...
      setStats({
        totalSurveys: projects?.length || 0, activeParticipants,
        totalResponses: totalResponseCount || 0, completionRate, recentResponses: enrichedRecent
      });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h1 className="text-2xl font-semibold tracking-tight text-neutral-900 mb-6">Analytics</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={s.label} className="bg-white rounded-xl p-4 border border-black/[0.04]">
            <s.icon size={16} className="text-emerald-600 mb-3" />
      ...
      <div className="bg-white rounded-xl border border-black/[0.04]">
        <div className="px-5 py-4 border-b border-black/[0.04]">
          <h3 className="text-[15px] font-semibold text-neutral-900">Recent Responses</h3>
        </div>
        {loading ? (
            ...
            {stats.recentResponses.map((response: any) => (
              <div key={response.id} className="px-5 py-3.5">
                <div className="flex justify-between items-start mb-1">
                  ...
                  <span className="shrink-0 ml-3 px-2 py-0.5 rounded-full text-[11px] font-medium bg-emerald-50 text-emerald-700">
          ...
          <div className="p-12 text-center">
            <BarChart3 className="mx-auto mb-3 text-neutral-300" size={32} />
            <p className="text-[13px] text-neutral-400">No responses yet. Share your surveys to start collecting data.</p>
          </div>
        )}
      </div>
    </div>
