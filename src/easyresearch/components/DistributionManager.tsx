...
<script src="${typeof window !== 'undefined' ? window.location.origin : ''}/embed.js" async></script>`;
  ...
  const tabs = [
    { id: 'link' as const, icon: Link2, label: t('dm.link') },
    { id: 'email' as const, icon: Mail, label: t('dm.emailTab') },
    { id: 'embed' as const, icon: Code, label: t('dm.embedTab') },
    { id: 'qr' as const, icon: QrCode, label: t('dm.qrTab') },
    { id: 'social' as const, icon: Share2, label: t('dm.socialTab') },
  ];

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-stone-800">{t('dm.title')}</h2>
        <div className="flex items-center gap-2 text-xs text-stone-500">
          <Users size={13} /> {t('dm.surveyCode')}: <code className="bg-stone-100 px-2 py-0.5 rounded font-mono">{surveyCode || 'N/A'}</code>
        </div>
      </div>
        ...
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium transition-all ${
              activeTab === tab.id ? 'bg-white shadow-sm text-stone-800' : 'text-stone-500 hover:text-stone-700'
            }`}>
      ...
      {activeTab === 'link' && (
        <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
          <div>
            <label className="text-xs font-medium text-stone-600 mb-1 block">{t('dm.surveyUrl')}</label>
            <div className="flex gap-2">
              ...
              <button onClick={() => copyToClipboard(buildUrl)}
                ...
                {copied ? <Check size={13} /> : <Copy size={13} />} {copied ? t('ss.copied') : t('ss.copy')}
              </button>
            </div>
          </div>

          <div className="border-t border-stone-100 pt-4">
            <h4 className="text-xs font-semibold text-stone-700 mb-3">{t('dm.utmParams')}</h4>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="text-[10px] text-stone-500 mb-0.5 block">{t('dm.source')}</label>
              ...
              <div>
                <label className="text-[10px] text-stone-500 mb-0.5 block">{t('dm.medium')}</label>
              ...
              <div>
                <label className="text-[10px] text-stone-500 mb-0.5 block">{t('dm.campaign')}</label>
          ...
          <div className="bg-stone-50 rounded-lg p-3">
            <h4 className="text-xs font-semibold text-stone-700 mb-2">{t('dm.accessMode')}</h4>
            <div className="flex gap-2">
      ...
      {activeTab === 'email' && (
        <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
          <div>
            <label className="text-xs font-medium text-stone-600 mb-1 block">{t('dm.recipients')}</label>
            ...
            <span className="text-[10px] text-stone-400 mt-1 block">
              {emailList.split('\n').filter(e => e.trim() && e.includes('@')).length} {t('dm.validEmails')}
            </span>
          </div>
          <div>
            <label className="text-xs font-medium text-stone-600 mb-1 block">{t('dm.subject')}</label>
            <input type="text" value={emailSubject} onChange={e => setEmailSubject(e.target.value)}
              className="w-full text-sm border border-stone-200 rounded-lg px-3 py-2" />
          </div>
          <div>
            <label className="text-xs font-medium text-stone-600 mb-1 block">{t('dm.message')}</label>
          ...
          <div className="flex gap-2">
            <button className="flex-1 py-2.5 bg-emerald-500 text-white rounded-lg text-xs font-medium hover:bg-emerald-600 transition-all flex items-center justify-center gap-1.5">
              <Mail size={13} /> {t('dm.sendInvitations')}
            </button>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
            <p className="text-xs text-amber-700">{t('dm.smtpWarning')}</p>
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-medium text-stone-600">{t('dm.iframeEmbed')}</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-medium text-stone-600">{t('dm.popupWidget')}</label>
      ...
      {activeTab === 'social' && (
        <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-3">
          <h4 className="text-xs font-semibold text-stone-700 mb-2">{t('dm.shareOnSocial')}</h4>
          {[
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-4">
        <h3 className="text-sm font-semibold text-stone-700 mb-3 flex items-center gap-1.5"><BarChart3 size={14} /> {t('dm.channelPerformance')}</h3>
        <div className="grid grid-cols-4 gap-3">
          {[
            { channel: t('dm.directLink'), clicks: 0, responses: 0 },
            { channel: t('dm.emailTab'), clicks: 0, responses: 0 },
            { channel: t('dm.embedTab'), clicks: 0, responses: 0 },
            { channel: t('dm.socialTab'), clicks: 0, responses: 0 },
          ].map(c => (
            <div key={c.channel} className="p-3 bg-stone-50 rounded-lg text-center">
              ...
              <p className="text-[10px] text-stone-400">{c.responses} {t('dm.responses')}</p>
            </div>
          ))}
        </div>
      </div>
