...
interface ComplianceData {
  date: string;
  expected_respons: number;
  completed_respons: number;
  missed_respons: number;
...
const ESMParticipantDashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Look up enrollment by project_id AND participant (user) id
  ...
  const renderSummaryTab = () => {
    ...
    const handleEdit = (instanceId: string) => {
      ...
      if (instance) {
        ...
        instance.responses.forEach(r => {
          if (r.response_text && String(r.response_text).trim().length > 0) {
            responsesMap[r.question_id] = r.response_text;
          } else if (r.response_value !== null && r.response_value !== undefined) {
            responsesMap[r.question_id] = typeof r.response_value === 'string' ? r.response_value : JSON.stringify(r.response_value);
          } else {
    ...
    const getAnswerText = (response: any) => {
      ...
      if (['single_choice', 'multiple_choice'].includes(question.question_type)) {
        ...
        let actualValue = response.response_value ?? response.response_text;
        if (actualValue && typeof actualValue === 'object') {
          if ('selected' in actualValue) actualValue = (actualValue as any).selected;
          else if ('option_id' in actualValue) actualValue = (actualValue as any).option_id;
          else if ('text' in actualValue) actualValue = (actualValue as any).text;
        }
        ...
        return opt?.text || (response.response_text || '') || String(actualValue ?? '');
      }
