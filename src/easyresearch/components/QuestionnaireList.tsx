...
import { Plus, Trash2, Clock, Bell, BellOff, ChevronDown, ChevronUp, Copy, GripVertical, Settings, FileText, Edit2, Users } from 'lucide-react';
...
export interface QuestionnaireConfig {
  ...
  // Schedule settings
  frequency: string; // 'hourly' | '2hours' | '4hours' | 'daily' | 'twice_daily' | 'weekly' | 'once'
  ...
  // Participant type assignments
  assigned_participant_types: string[]; // IDs of participant types
  // Order
  order_index: number;
}
...
const QuestionnaireList: React.FC<QuestionnaireListProps> = ({
  ...
  const addQuestionnaire = () => {
    ...
    const newQ: QuestionnaireConfig = {
      ...
      assigned_participant_types: participantTypes.map(pt => pt.id), // All by default
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      onClick={(e) => { e.stopPropagation(); onEditQuestions(q.id); }}
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); duplicateQuestionnaire(q); }}
                  ...
                  <div className="border-t border-stone-100 px-4 py-4 space-y-4 bg-stone-50/30">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          value={q.estimated_duration}
                          onChange={(e) => updateQuestionnaire(q.id, { estimated_duration: parseInt(e.target.value) || 5 })}
                          className="w-full px-3 py-2 rounded-xl text-[13px] border border-stone-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400"
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        placeholder="E.g., Log your activities and mood every hour..."
                      />
                    </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <CustomDropdown
                            ...
                            value={q.frequency}
                            onChange={(v) => updateQuestionnaire(q.id, { frequency: v })}
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1.5">
                            <input
                              type="time"
                              value={q.time_windows[0]?.start || '09:00'}
                              onChange={(e) => updateQuestionnaire(q.id, { time_windows: [{ ...q.time_windows[0], start: e.target.value }] })}
                              className="flex-1 px-2 py-1.5 rounded-lg text-[12px] border border-stone-200"
                            />
                            <span className="text-[11px] text-stone-400">to</span>
                            <input
                              type="time"
                              value={q.time_windows[0]?.end || '21:00'}
                              onChange={(e) => updateQuestionnaire(q.id, { time_windows: [{ ...q.time_windows[0], end: e.target.value }] })}
                              className="flex-1 px-2 py-1.5 rounded-lg text-[12px] border border-stone-200"
                            />
                          </div>
                        </div>
                    ...
                    <div className="bg-white rounded-xl border border-stone-200 p-3 space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-[12px] font-medium text-stone-700">Push Notifications</p>
                          <p className="text-[11px] text-stone-400">Remind participants when it's time to fill</p>
                        </div>
                        <button
                          onClick={() => updateQuestionnaire(q.id, { notification_enabled: !q.notification_enabled })}
                          className={`relative w-10 h-5 rounded-full transition-colors shrink-0 ${q.notification_enabled ? 'bg-emerald-500' : 'bg-stone-200'}`}
                        >
                      ...
                      {q.notification_enabled && (
                        <div>
                          ...
                          <input
                            type="number"
                            value={q.notification_minutes_before}
                            onChange={(e) => updateQuestionnaire(q.id, { notification_minutes_before: parseInt(e.target.value) || 5 })}
                            className="w-24 px-2 py-1.5 rounded-lg text-[12px] border border-stone-200"
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-white rounded-xl border border-stone-200 p-3 space-y-3">
                      <h5 className="text-[12px] font-semibold text-stone-600 uppercase tracking-wider flex items-center gap-1.5">
                        <BellOff size={12} /> Do Not Disturb
                      </h5>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-[12px] font-medium text-stone-700">Allow DND</p>
                          <p className="text-[11px] text-stone-400">Participants can set quiet hours</p>
                        </div>
                        <button
                          onClick={() => updateQuestionnaire(q.id, { dnd_allowed: !q.dnd_allowed })}
                          className={`relative w-10 h-5 rounded-full transition-colors shrink-0 ${q.dnd_allowed ? 'bg-emerald-500' : 'bg-stone-200'}`}
                        >
                      ...
                      {q.dnd_allowed && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <input
                            type="time"
                            value={q.dnd_default_start}
                            onChange={(e) => updateQuestionnaire(q.id, { dnd_default_start: e.target.value })}
                            className="px-2 py-1.5 rounded-lg text-[12px] border border-stone-200"
                          />
                          <span className="text-[11px] text-stone-400">to</span>
                          <input
                            type="time"
                            value={q.dnd_default_end}
                            onChange={(e) => updateQuestionnaire(q.id, { dnd_default_end: e.target.value })}
                            className="px-2 py-1.5 rounded-lg text-[12px] border border-stone-200"
                          />
                        </div>
                      )}
                    ...
                    {participantTypes.length > 0 && (
                      <div className="bg-white rounded-xl border border-stone-200 p-3 space-y-2">
                        <h5 className="text-[12px] font-semibold text-stone-600 uppercase tracking-wider flex items-center gap-1.5">
                          <Users size={12} /> Assigned Participant Types
                        </h5>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const newAssigned = assigned
                                    ? q.assigned_participant_types.filter(id => id !== pt.id)
                                    : [...q.assigned_participant_types, pt.id];
                    ...
                    <button
                      onClick={() => onEditQuestions(q.id)}
                      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-[13px] font-medium border-2 border-dashed border-emerald-300 text-emerald-600 hover:bg-emerald-50 transition-colors"
                    >
                      <Edit2 size={14} /> Edit Questions ({q.questions.length})
                    </button>
                  </div>
                )}
              </div>
