...
const ResponsesPage: React.FC = () => {
  ...
  const loadResponses = async () => {
    try {
      ...
      let query = supabase.from('survey_respons')
      ...
      if (responseData) {
        ...
        setResponses(rawResponses.map(r => ({
          ...r, enrollment: enrollmentById.get(r.enrollment_id),
          question: questionById.get(r.question_id), project: projectById.get(r.project_id)
        })) as any);
      }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-neutral-900">Responses</h1>
          <p className="text-[13px] text-neutral-400 mt-1">View and export participant responses</p>
        </div>
        <button
          onClick={exportResponses}
          className="px-3.5 py-1.5 rounded-lg text-[13px] font-medium text-white bg-emerald-600 hover:bg-emerald-700 flex items-center gap-1.5 transition-colors"
        >
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <select
          ...
          className="px-3 py-1.5 rounded-lg text-[13px] border border-black/[0.08] bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
        >
      ...
      <div className="bg-white rounded-xl border border-black/[0.04] overflow-hidden">
        {responses.length === 0 ? (
          <div className="p-16 text-center">
            ...
            <h3 className="text-[15px] font-semibold text-neutral-900 mb-1">No Responses Yet</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-black/[0.04] bg-neutral-50/50">
                  <th className="text-left px-4 py-3 text-[12px] font-medium text-neutral-500 uppercase tracking-wider">Time</th>
                  <th className="text-left px-4 py-3 text-[12px] font-medium text-neutral-500 uppercase tracking-wider">Project</th>
                  <th className="text-left px-4 py-3 text-[12px] font-medium text-neutral-500 uppercase tracking-wider">Participant</th>
                  <th className="text-left px-4 py-3 text-[12px] font-medium text-neutral-500 uppercase tracking-wider">Question</th>
                  <th className="text-left px-4 py-3 text-[12px] font-medium text-neutral-500 uppercase tracking-wider">Answer</th>
                </tr>
              </thead>
                ...
                {responses.map((response) => (
                  <tr key={response.id} className="hover:bg-neutral-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <p className="text-[13px] text-neutral-600 line-clamp-1">{response.question?.question_text || 'Unknown'}</p>
                    </td>
                    <td className="px-4 py-3 text-[13px] font-medium text-neutral-900 max-w-xs truncate">
